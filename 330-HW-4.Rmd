...
```
This plot shows several of the variables and their corr values.  We are mostly interested in the how the variables relate to body fat percentage.  We can see that they look fairly linear.  That is, as the variable increases, so does body fat percentage for all variables except height.  Looking at the corr values we can see that for body fat percentage they are all positive except for height once again.  We will pull those out to get a better look.
...
```
Here we can see the corr between the variable and body fat percentage. Most of them are fairly strong except age, height, ankle, forearm and wrist.  With height there seems to be almost no correlation.
...
As a person's age increases by 1 year, a person's body fat is expected to increase by 0.0568 percent, on average, assuming all other explanitory variables are held constant..
...
As a person's wrist cicumference increases by 1 centimeter, a person's body fat is expected to decrease by 1.4766 percent, on average, assuming all other explanitory variables are held constant.
...
```
Using a cross-validation study of 500 samples we are able to get a sense of how accurate our model is at predicting body fat percentage.  The bias is `r bias_mean`.  This means our model typically predits body fat percentage `r bias_mean` above the actual value on average. That value is relatively small so we feel our model isn't very biased. The root prediction mean squared error value of `r rpmse_mean` tells us our predictions miss the mark by an average of `r rpmse_mean`. 
...
Our model has an $R^2$ value of `r r2`.  This means that the variables together are able to explain `r r2*100`% of the variation in body fat percentage. This means our model does a good job at fitting the data.  It is not the best because there is still variation on body fat we can't explain, but it is enough to feel comfortable using our model. 
...
```
Using the model we get a prediction of `r predict_values[1]`.  This means for a person with the measurments given, we would expect them to have `r predict_values[1]`% body fat.  We are 95% confident that the person's actual body fat percentage would be between `r predict_values[2]`% and `r predict_values[3]`%.  
